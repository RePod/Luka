    ...
    'search' => sub {
      ...
      foreach(@{$results{'Search'}}) {
        ...
        if(!$_[4]) {
          ...
          &{$utility{'Fancify_say'}}($_[0],$_[1],"[http://www.imdb.com/title/$imdb{imdbID}] [\x04$imdb{Title}\x04 $imdb{Year}] [\x04$imdb{Genre}\x04] [\x04$imdb{imdbRating}\x04/\x0410\x04] [\x04$imdb{Runtime}\x04] $imdb{Plot}");
        }
        $i++; if((!$_[3]) || ($i > $_[3])) { last; }
      }
    },
